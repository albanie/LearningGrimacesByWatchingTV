function [net, info] = fer_baselines(varargin)
% FER_BASELINES train simple baselines on FER
%   [NET, INFO] = FER_BASELINES(VARARGIN)
%
%   FER_BASELINES(..'name', value) accepts the following
%   options:
%
%   `dropoutRate` :: 0.5
...
% Licensed under The MIT License [see LICENSE.md for details]
  ...
  opts.imdbPath = fullfile(vl_rootnn, 'data/grimaces/fer2013/imdb.mat');
  ...
  getBatch = @(i,b) getBatchFer(i, b, opts, dag) ;
...
% -----------------------------------------------------------
  expDir = fullfile(vl_rootnn, 'data/grimaces/fer2013', opts.modelName) ;
...
% --------------------------------------------------------------------
function inputs = getBatchFer(imdb, batch, opts, dag)
% --------------------------------------------------------------------
% GETBATCHFER - load a batch of data from FER
%    INPUTS = GETBATCHFER(IMDB, BATCH, OPTS, DAG) returns the inputs and
...
%    finding them in the IMDB data structure.
  ...
  for i = 1:numel(batch)
    if opts.dataAug & strcmp(dag.mode, 'normal')
      tmp = augs(:,:,i) ;
    else
